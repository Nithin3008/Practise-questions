...
const mostActivePerson = (arr) => {
    ...
    let high = 0
    const arr2 = arr.reduce((active, val) => {

        let sum = val.hours.reduce((attr, val) => attr + val, 0)
        if (sum > high) {
            high = sum;
            return val
        }
    ...
    console.log(arr2)
    // const highesthrs = arr2.reduce((attr, val) => {
    //     if (attr > val) {
    //         return attr;
    //     } else {
    //         return (attr = val);
    //     }
    // }, 0);
    // const indexofHighHrs = arr2.indexOf(highesthrs);
    // return arr[indexofHighHrs];
};
